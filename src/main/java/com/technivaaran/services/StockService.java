...
public class StockService {
        ...
        public List<StockResponseDto> getStockHeader() {
                ...
                // .add(stockHeaderResponseMapper.convertToDto(stockHeader,
                // StockTransactionType.NORMAL)));
                return stockResponseDtos;
        }

        public Page<InventoryRow> getInventoryData(int page, int size) {
                Pageable pageable = PageRequest.of(page, size, Sort.by("id").descending());
                return stockHeaderRepository.getInventoryData(pageable);
        }

        public Page<InventoryRow> searchInventoryData(int page, int size, String query) {
                Pageable pageable = PageRequest.of(page, size, Sort.by("id").descending());
                return stockHeaderRepository.searchInventoryData(query, pageable);
        }
        ...
        public ResponseEntity<OmsResponse> createStockEntry(StockRequestDto stockRequestDto) {
                ...
                if (configOp.isEmpty()) {
                        return new ResponseEntity<>(
                                        OmsResponse.builder().message("Invalid configuration received.").build(),
                ...
                if (vendorOp.isEmpty()) {
                        return new ResponseEntity<>(
                                        OmsResponse.builder().message("Invalid Vendor name received.").build(),
                                        HttpStatus.BAD_REQUEST);
                }
                ConfigDetailsEntity configEntity = configOp.get();
                // Optional<StockHeader> stockHeaderOp =
                // findStockHeaderByLocationAndModelAndPartAndConfigAndVendorAndBuyPrice(
                // Optional<StockHeader> stockHeaderOp =
                ...
                if (stockHeaderOp.isPresent()) {
                        ...
                        if (stockHeader.getClosingQty() > 0) {
                                return new ResponseEntity<>(
                                                // OmsResponse.builder().message("Stock data already available click on
                                                // table row to update data.")
                                                OmsResponse.builder().message(
                                                                "Stock data already available click on inventory table row to update data.")
                                                                .build(),
                                                HttpStatus.BAD_REQUEST);
                        } else {
                                ...
                                double avgPrice = calculateAveragePrice(configEntity.getId(),
                                                ...
                                                stockRequestDto.getBuyPrice(), stockRequestDto.getQty(),
                                                stockRequestDto.getDetails(),
                                ...
                                updateAverageBuyAndSellPrice((float) avgPrice, stockRequestDto.getSellPrice(),
                                                configEntity.getId(),
                                                configEntity.getPartEntity().getItemMaster().getId(),
                                                configEntity.getPartEntity().getId(),
                ...
                return new ResponseEntity<>(OmsResponse.builder().message(STOCK_UPDATE_SUCCESS)
                                .data(stockHeaderResponseMapper.convertToDto(stockHeader, StockTransactionType.NORMAL))
                                .build(),
                                HttpStatus.OK);
        ...
        public double calculateAveragePrice(Long configDetailsId, Long itemMasterId, Long partId, float buyPrice,
                ...
                var sumAndRowCountForBuyPrice = getSumAndRowCountForBuyPrice(configDetailsId, itemMasterId, partId,
                                details,
                                remark);
                ...
                if (useOrg && (itemQty + qty + orgQty) != 0) {
                        return Math.ceil((sumBuyPrice + (buyPrice * qty) + (orgQty * orgBuyPrice))
                                        / (itemQty + qty + orgQty));
                }
        ...
        public Optional<StockHeader> findStockHeaderByLocationAndModelAndPartAndConfigAndVendorAndDetails1(
                        StorageLocationEntity storageLocationEntity, ConfigDetailsEntity configDetailsEntity,
                        VendorEntity vendor,
                        String details) {
                ...
                return stockHeaderRepository
                                .findByStorageLocationAndItemMasterAndPartEntityAndConfigDetailsEntityAndVendorAndDetailsAndRowDelStatus(
                                                ...
                                                configDetailsEntity.getPartEntity().getItemMaster(),
        ...
        public double getAveragePriceForConfig(Long configDetailsId, Long itemMasterId, Long partId, String details,
                ...
                return stockHeaderRepository.getAveragePriceForConfig(configDetailsId, itemMasterId, partId, details,
                                remark);
        }

        public void updateAverageBuyAndSellPrice(float buyPrice, float sellPrice, Long configDetailsId,
                        Long itemMasterId,
                        Long partId, String details, String remark) {
                try {
                        stockHeaderRepository.updateAverageBuyAndSellPrice(buyPrice, sellPrice, configDetailsId,
                                        itemMasterId,
        ...
        @Transactional
        public ResponseEntity<OmsResponse> updateStockHeaderAndStockDetaisById(long id,
                ...
                if (stockHeaderOp.isEmpty()) {
                        return new ResponseEntity<>(
                                        OmsResponse.builder().message("Invalid stock header received.").build(),
                                        HttpStatus.BAD_REQUEST);
                }
                ...
                Optional<StockHeader> updateStockHeaderOp = findByStorageLocationAndItemMasterAndPartEntityAndConfigDetailsEntityAndVendorAndRemark(
                                ...
                                stockRequestDto.getUpdatedDetails(),
                ...
                if (updateStockHeaderOp.isPresent()) {
                        double avgPrice = 0;
                        if (updateStockHeaderOp.get().getId().equals(stockHeader.getId())) { // if we are updating same
                                                                                             // stockHeader
                                // if
                                // (stockRequestDto.getUpdatedDetails().equalsIgnoreCase(stockHeader.getDetails()))
                                // {
                                ...
                                // responseEntity = updateStockHeaderAndStockDetais(stockHeader,
                                ...
                                // stockRequestDto.getUpdatedRemark());
                                // } else {
                                // // in this scenario we are updating the existing entry after check if the
                                // details are mismatching
                                ...
                                responseEntity = updateStockHeaderAndStockDetais(updateStockHeaderOp.get(),
                                                stockRequestDto.getStockType(),
                                                stockHeader.getClosingQty() + stockRequestDto.getQty(),
                                                stockRequestDto.getSellPrice(),
                                                stockRequestDto.getUpdatedBuyPrice(), user,
                                                StockTransactionType.CONVERT,
                                                stockRequestDto.getUpdatedRemark());
                                ...
                                StockDetails stockDetails = createStockDetails(0, 0, user, 0,
                                                stockHeader.getClosingQty());
                        ...
                        return createNewStockEntry(stockRequestDto, stockHeader, storageLocationOp.get(),
                                        vendorOp.get(),
                                        configEntity, user);
                }
        }

        private ResponseEntity<OmsResponse> createNewStockEntry(StockRequestDto stockRequestDto,
                ...
                return new ResponseEntity<>(OmsResponse.builder().message(STOCK_UPDATE_SUCCESS)
                                .data(stockHeaderResponseMapper.convertToDto(stockHeaderNew,
                                                StockTransactionType.CONVERT))
                                .build(), HttpStatus.OK);
        ...
        public ResponseEntity<OmsResponse> updateStockHeaderAndStockDetais(StockHeader stockHeader,
                ...
                switch (StockType.valueOf(stockType.toUpperCase())) {
                        case IN: {
                                if ((stockHeader.getClosingQty() + quantity) >= 0) {
                                        ...
                                        if (remark != null && !remark.isEmpty()) {
                                                Optional<RemarkEntity> remarkOp = remarkService
                                                                .findRemarkByRemarkText(remark);
                                        ...
                                        return new ResponseEntity<>(
                                                        OmsResponse.builder()
                                                                        .message("Invalid stock quantity received.")
                                                                        .build(),
                                                        HttpStatus.BAD_REQUEST);
                                }
                                break;
                        ...
                        case RETURN: {
                                return new ResponseEntity<>(
                                                OmsResponse.builder().message("Invalid stock type received.").build(),
                                                HttpStatus.BAD_REQUEST);
                        }
                        default:
                                return new ResponseEntity<>(
        ...
        private ResponseEntity<OmsResponse> updateStock(StockHeader stockHeader, StockDetails stockDetails,
                ...
                return new ResponseEntity<>(OmsResponse.builder().message(STOCK_UPDATE_SUCCESS)
                                .data(stockHeaderResponseMapper.convertToDto(stockHeader, stockTransactionType))
                                .build(),
                                HttpStatus.OK);
        }
